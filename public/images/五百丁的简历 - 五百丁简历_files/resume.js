 ...
 *		自定义多项属性值
 *			class：resume_custom_item；for-key（模块id）：属性值；resume_item_items：多项值（item1,item2,item3,item4,item5,item6...）--#@jeremy
 *		图标项：
...
$(function() {	
	 ...
	 * 禁止时间，单位，职位做回车操作
	 */
	$(".resume_time,.resume_unit,.resume_job,.resume_tilte_name,.resume_msg").live("keydown",function(event){
	...
	if(pageSize != nowPageSize) {
		...
		for(var index = 1; index < pageSize; index++) {
			if(index!=pageSize){
				...
				if(resume_version!=null&&resume_version!=""&&resume_version=="v4"){//v4版本分页符高40px
					pageBreakObj.css({"top" : ((index * pageHeight)-20) + "px"});
				}else{
...
function resumeSave(showNotice) {
	var json = getResume();
	var copyResumeContentId=$("#copyResumeContentId").val();
	...
	$.ajax({type : "post",
		async : false,
		url : saveUrl,
		data : {"id" : themeMessage.resumeBankItemId, "memberId" : member.id, "resumeId" : member.resumeId,"copyResumeContentId" :copyResumeContentId, "json" : JSON.stringify(json)},
		success : function(message) {
			...
			if(message.type == "success") {
				...
				var $doc_dlown_btn=$(".doc_dlown_btn");
				if($doc_dlown_btn.length>0){
					if($doc_dlown_btn.attr("href").indexOf("javascript")==-1){
						var timestamp=new Date().getTime();
						var download_url=$("#hidden_data_download_url").val();
						if(download_url==""||download_url==null||download_url==undefined){
							download_url= wbdcnf.base;
						}
						var dlown_url=download_url+"/editresume/export/500d_"+member.resumeId+"_"+themeMessage.resumeBankItemId+"_"+member.id+"_"+timestamp+".pdf?downmsg="+$doc_dlown_btn.attr("data_downmsg");
						$doc_dlown_btn.attr("href",dlown_url);
					}
				}
				return true;
			} else {
...
function resumeAddResumeItems() {
	$(".resume_add").live("click", function() {
		...
		else if(forkey == "resume_works")//作品集
			html = addMessage.get_resume_works();
 ...
 * 获取基本信息
 */
function getResumeCoverMsg(resume) {
	var messages = {};
	$(".resume_cover_msg").each(function(index, ele) {
		ele = $(ele);
		var key = ele.attr("for-key");
		var fortype = ele.attr("for-type");
		var forvalue = ele.attr("for-value");
	...
	//console.log(resume);
	resume["cover_messages"] = messages;
}
/**
 * 获取自定义
 */
 ...
 * 获取自定义多项属性值
 */
function getResumeCustomItem(resume) {
	$(".resume_custom_item").each(function(index, ele) {
		ele = $(ele);
		var eles = ele.find(".resume_item_items");
		//判断一下版本
		var resume_version=$("#resume_body").attr("version");
		...
		eles.each(function(j, ele_item) {
			ele_item = $(ele_item);
			var item = {};
			var resume_custom_item_list=ele_item.find(".resume_custom_item_list");
			resume_custom_item_list.each(function(z, ele_item_list) {
				ele_item_list=$(ele_item_list);
				if(ele_item_list.attr("for-type")=="value"){
					item[ele_item_list.attr("for-key")] = clearText(ele_item_list.attr("for-value"));
				}else{
					item[ele_item_list.attr("for-key")] = clearText(ele_item_list.html());
				}
			});
			items[j] = item;
		});
