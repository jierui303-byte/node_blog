$(document).ready(function(){
	...
	function resumeModal(){
		$(".saveBtn").click(function(){
			//判断是否是第一次保存，是的话则提示简历名称填写提示框
			var save_flag=$("input.resumetitle").attr("data_flag");
			if(save_flag!=null&&save_flag!=""&&save_flag!=undefined&&save_flag=="false"){
				if($("#resumeTitle").attr("data_value")==$("input.resumetitle").val()){//简历标题是否有修改
...
//	          $('#getCroppedCanvasModal').modal().find('.modal-body').html(result);
    ...
    $(".works_image_list").live("click",function(){
    	var $this=$(this);
    	$(".works_image_list").find("img").removeClass("select_upload_image");
    	$this.find("img").addClass("select_upload_image");
    	$("#works_image").modal("show");
    	cropper_image(200/230);
//    	var show_imge=$this.find("img");
//    	layer.open({
//    		  type: 1,
//    		  title: false,
//    		  closeBtn: 0,
//    		  area: [show_imge.width*5,show_imge.height*5],
...
//    	});
	...
	//图片裁剪
	function cropper_image (imgae_aspectRatio,preview_class) {
	    var $image =$(".cut_works_image>img"),
	        options = {
	          aspectRatio: imgae_aspectRatio,
	          preview:'.works_image_preview',
	          crop: function (data) {
	          }
	        };
	    $image.on({
		...
		$(".works_image span[data-method]").click(function(){
			var data = $(this).data(),
	          $target,
	          result;
			  if (data.method) {
		...
		$("#upload_image_btn").click(function(){
			 var $image = $('.cut_works_image>img');
			 var $isCoopered=$(".cut_works_image").find("div");//判断是否有上传
			 if($isCoopered==null||$isCoopered.length==0){
				 layer.msg("请先上传图片!");
				 return;
			 }
			 var upload_file =document.getElementById("worksImage").files;
			 if(upload_file.length>0){
				 var upload_file_size=upload_file[0].size/1024;
				 if((upload_file_size/1024)>1){
					layer.msg("请上传小于2M的图片");
			 ...
			 $(".zx-loading").show();
			 $("#upload_image_btn").prop("disabled",true);
			 //图片上传---直接上传裁剪的照片
			 var image_cut_data = $image.cropper("getCroppedCanvas");
			 var image_cut_data_data=image_cut_data.toDataURL("image/jpeg");
			 //把裁剪好的图片上传
				 ...
				 if(result == "error") {
					...
					alert("上传头像请先登录！");
			 		...
			 		layer.msg("裁剪出错请重新裁剪！");
			 		 ...
			 		 $("img.select_upload_image").attr("src",result);
			 		 $("img.select_upload_image").closest("div").attr("for-value",result);
			 	}
				$("#upload_image_btn").prop("disabled",false);
				$(".zx-loading").hide();
				$("#works_image").modal("hide");
			 });
		 });
	};
	function mbList(){
	...
	function reload_template_list(){
		...
		if(select_template_page==1){
			$("#data_template_list").load("/editresume/select_template/",{"type":current_resume_type,"resumeBankType":current_resume_bank_type,"pageNum":select_template_page,"sort":data_sort,"keyword":keyword},function(){
			 ...
			 $.get("/editresume/select_template/",{"type":current_resume_type,"resumeBankType":current_resume_bank_type,"pageNumber":select_template_page,"sort":data_sort,"keyword":keyword},function(result){
